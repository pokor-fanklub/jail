...
void Runner::monitorProcess() {
    ...
    for(;;) {
        ...
        if(should_exit) {
            ...
            if (WIFEXITED(wait_s)) {
                ...
                output.exit_ok(WEXITSTATUS(wait_s));
                ...
                output.exit_killsig(WTERMSIG(wait_s), true);
            } else if (WIFSIGNALED(wait_s)) {
                std::cout<<"terminated by signal nr. "<<WTERMSIG(wait_s)<<'\n';
                output.exit_killsig(WTERMSIG(wait_s), false);
            }
        ...
        if(exit_flag) {
            ...
            output.set_state(uregs.orig_rax, uregs.rip);
            output.set_mem(pidResources());
        }
        ...
        if(!exec_flag && !exit_flag) {
            ...
            // this signal should be SIGTRAP generated by ptrace
    ...
    output.set_time_limit((!time_limit.verify_insn_limit() ? 
        TimeLimit::KillReason::INSTR_LIM_EXCD
        : static_cast<TimeLimit::KillReason>(time_limit.get_killed())));

    std::cout<<perf.readInstructions()<<" instructions"<<'\n';
    output.set_instructions(perf.readInstructions());
